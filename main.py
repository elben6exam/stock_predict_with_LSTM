...
class Config:
    ...
    do_continue_train = (
        False  # 每次训练把上一次的final_state作为下一次的init_state，仅用于RNN类型模型，目前仅支持pytorch
    )
    ...
    do_train_visualized = (
        False  # 训练loss可视化，pytorch用visdom，tf用tensorboardX，实际上可以通用, keras没有
    )
...
class Data:
    ...
    def get_train_and_valid_data(self):
        ...
        if not self.config.do_continue_train:
            ...
            train_x = [
                feature_data[i : i + self.config.time_step]
                for i in range(self.train_num - self.config.time_step)
            ]
            train_y = [
                label_data[i : i + self.config.time_step]
                for i in range(self.train_num - self.config.time_step)
            ]
        else:
            ...
            train_x = [
                feature_data[
                    start_index
                    + i * self.config.time_step : start_index
    ...
    def get_test_data(self, return_label_data=False):
        feature_data = self.norm_data[self.train_num :]
        sample_interval = min(
            feature_data.shape[0], self.config.time_step
        )  # 防止time_step大于测试集数量
        self.start_num_in_test = (
            feature_data.shape[0] % sample_interval
        ...
        test_x = [
            feature_data[
                self.start_num_in_test
                + i * sample_interval : self.start_num_in_test
                + (i + 1) * sample_interval
            ]
        ...
        if return_label_data:  # 实际应用中的测试集是没有label数据的
            label_data = self.norm_data[
                self.train_num + self.start_num_in_test :,
                self.config.label_in_feature_index,
            ]
...
def draw(config: Config, origin_data: Data, logger, predict_norm_data: np.ndarray):
    ...
    predict_data = (
        predict_norm_data * origin_data.std[config.label_in_feature_index]
        + origin_data.mean[config.label_in_feature_index]
    )  # 通过保存的均值和方差还原数据
    assert (
        label_data.shape[0] == predict_data.shape[0]
    ), "The element number in origin and predicted data is different"

    label_name = [
        origin_data.data_column_name[i] for i in config.label_in_feature_index
    ]
    ...
    loss = np.mean(
        (label_data[config.predict_day :] - predict_data[: -config.predict_day]) ** 2,
        axis=0,
    )
    loss_norm = loss / (origin_data.std[config.label_in_feature_index] ** 2)
    logger.info(
        "The mean squared error of stock {} is ".format(label_name) + str(loss_norm)
    )
    ...
    if not sys.platform.startswith(
        ...
        for i in range(label_column_num):
            ...
            plt.title(
                "Predict stock {} price with {}".format(
                    label_name[i], config.used_frame
                )
            )
            logger.info(
                "The predicted stock {} for the next {} day(s) is: ".format(
                    label_name[i], config.predict_day
                )
